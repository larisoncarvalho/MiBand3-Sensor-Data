...
class MiBand2(Peripheral):
    ...
    def __init__(self, mac_address, timeout=0.5, debug=False):
        ...
        self._char_sensor_measure = self.svc_1.getCharacteristics(UUIDS.CHARACTERISTIC_SENSOR_DATA)[0]
    ...
    def start_raw_data_realtime(self, heart_measure_callback=None, heart_raw_callback=None, accel_raw_callback=None):
        ...
        while True:
            ...
            # if (time.time() - t) >= 12:
                #char_ctrl.write(b'\x16', True)
            char_sensor.write(b'\x16')
                # t = time.time()

    def get_raw_gyro_start(self, accel_raw_callback=None):

        if accel_raw_callback:
            self.accel_raw_callback = accel_raw_callback

        char_sensor1 = self.svc_1.getCharacteristics(UUIDS.CHARACTERISTIC_SENSOR_DATA)[0]
        char_sens_d1 = char_sensor1.getDescriptors(forUUID=UUIDS.NOTIFICATION_DESCRIPTOR)[0]

        char_sensor2 = self.svc_1.getCharacteristics(UUIDS.CHARACTERISTIC_SENSOR_CONTROL)[0]

        # char_sensor2.write(b'\x01\x01\x19')
        char_sensor2.write(b'\x01\x03\x19')

        char_sensor2.write(b'\x02')
        # IMO: start notifications from sensors
        char_sens_d1.write(b'\x01\x00', True)
        ...
        while True:
            ...
            # send ping request every 12 sec
            if (time.time() - t) >= 12:
                # char_sensor2.write(b'\x16', True)
                t = time.time()

    def test(self, heart_measure_callback=None, heart_raw_callback=None, accel_raw_callback=None):
        char_m = self.svc_heart.getCharacteristics(UUIDS.CHARACTERISTIC_HEART_RATE_MEASURE)[0]
        char_d = char_m.getDescriptors(forUUID=UUIDS.NOTIFICATION_DESCRIPTOR)[0]
        char_ctrl = self.svc_heart.getCharacteristics(UUIDS.CHARACTERISTIC_HEART_RATE_CONTROL)[0]
        ...
        # char_sensor = self.svc_1.getCharacteristics(UUIDS.CHARACTERISTIC_SENSOR_CONTROL)[0]
